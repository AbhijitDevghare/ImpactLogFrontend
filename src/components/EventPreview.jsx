...
import { unregisterEvent } from "../redux/slices/EventSlice";
...
export default function EventPreview() {
  ...
  const { upcomingEvents = [] } = useSelector((s) => s.event);
  ...
  if (!state) {
    return (
      <div className="text-center py-12 text-red-400">
        ...
        <button onClick={() => navigate(-1)} className="ml-2 text-purple-400 underline">
          Go Back
        </button>
      </div>
    );
  ...
  const reg = state;               // { id, eventId, registeredAt, event: { ... } }
  const ev  = reg.event;           // actual event
  ...
  const handleUnregister = async () => {
    ...
    try {
      await dispatch(
        unregisterEvent({registrationId:reg.id })
      ).unwrap();
