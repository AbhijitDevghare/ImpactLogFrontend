...
export const getCommunities = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get("http://localhost:3000/users/communities", {
        withCredentials: true,
      });
...
export const resetPassword = createAsyncThunk("/auth/resetPassword", async ({ resetToken, newPassword }, { rejectWithValue }) => {
  try {
    const res = axiosInstance.put(`/user/auth/password/ reset/${resetToken}`, { password: newPassword });
    toast.promise(res, {
...
export const sendOtp = createAsyncThunk(
  ...
  async ({ phoneNumber, email }, { rejectWithValue }) => {
    try {
      const response = await axios.post("http://localhost:3000/auth/password/forgot/send-otp", {
        phoneNumber,
        email,
      });
...
export const verifyOtp = createAsyncThunk(
  ...
  async ({ phoneNumber, email, otp, password }, { rejectWithValue }) => {
    try {
      const response = await axios.post("http://localhost:3000/auth/password/reset/verify-otp", {
        phoneNumber,
        email,
        otp,
        password,
...
export const fetchUserProfile = createAsyncThunk("/user/fetchProfile", async (userId) => {
    try {
        const res = axiosInstance.get(`/users/profile/${userId}`);
        return (await res).data;
    } catch (error) {
...
export const followUser = createAsyncThunk("/users/follow", async (userId) => {
    try {
        const res = axiosInstance.post(`users/${userId}/follow`);
        toast.promise(res, {
...
export const unfollowUser = createAsyncThunk("/user/unfollow", async (userId) => {
    try {
        const res = axiosInstance.delete(`users/${userId}/unfollow`);
        toast.promise(res, {
...
export const verifyEmail = createAsyncThunk(
  ...
  async (email, { rejectWithValue }) => {
    try {
      const response = await axios.post('http://localhost:3000/auth/password/forgot/verify-email', { email });
      return response.data;
    } catch (error) {
